...
/// (Graphite in dark, Paper in light) chosen in Speed display settings.
struct Palette: Equatable, Sendable {
    ...
    var text3: Color
    var line: Color
    /// Round control fill on pages.
    var fill: Color
    /// Inactive switch track, off meter segments.
    var fill2: Color
    /// Selected segment in a segmented control.
    var seg: Color
    /// Dashboard module panel.
    var panel: Color
    /// Button fill inside a panel.
    var pbtn: Color
    /// Roads on the map module.
    var road: Color
    var warn: Color
    var danger: Color
    var isDark: Bool

    var speedRegular: Font.Weight { isDark ? .regular : .medium }
    var speedBold: Font.Weight { isDark ? .semibold : .bold }
    ...
    static func make(dark: Bool, ground: SpeedGround) -> Palette {
        ...
        case (true, .pure):
            Palette(
                ...
                fill2: Color(hex: 0x2E2E2D), seg: Color(hex: 0x343433), panel: Color(hex: 0x0F0F0F),
                pbtn: Color(hex: 0x1E1E1E), road: Color(hex: 0x1F1F1E),
                warn: Color(hex: 0xF0A83A), danger: Color(hex: 0xFF6A5E), isDark: true
            )
        case (true, .soft):
            Palette(
                ...
                fill2: Color(hex: 0x34363A), seg: Color(hex: 0x3A3C40), panel: Color(hex: 0x1A1B1E),
                pbtn: Color(hex: 0x2A2C30), road: Color(hex: 0x2A2C30),
                warn: Color(hex: 0xF0A83A), danger: Color(hex: 0xFF6A5E), isDark: true
            )
        case (false, .pure):
            Palette(
                ...
                fill2: Color(hex: 0xD6D6D2), seg: Color(hex: 0xFFFFFF), panel: Color(hex: 0xF4F4F2),
                pbtn: Color(hex: 0xE6E6E2), road: Color(hex: 0xFFFFFF),
                warn: Color(hex: 0x9A5200), danger: Color(hex: 0xC42A1E), isDark: false
            )
        case (false, .soft):
            Palette(
                ...
                fill2: Color(hex: 0xD3D2CC), seg: Color(hex: 0xFFFFFF), panel: Color(hex: 0xEBEAE5),
                pbtn: Color(hex: 0xDDDCD6), road: Color(hex: 0xFAF9F6),
                warn: Color(hex: 0x9A5200), danger: Color(hex: 0xC42A1E), isDark: false
            )
        }
    }
...
extension SpeedTint {
    /// Speed, gear and accent color. Controls always stay neutral.
